...
int main(int argc, char** argv) {
	...
	for(i=0; i<height*3; i+=3) { 
		for(j=0; j<width*3; j+=3) {
			...
			outimg[(j*xFactor+3)+(width*xFactor*i*yFactor)] = (inimg[j+(i*width)] + inimg[(j+3)+(i*width)]) / 2;
			outimg[(j*xFactor+3)+(width*xFactor*i*yFactor)+1] = (inimg[j+(i*width)+1] + inimg[(j+3)+(i*width) + 1]) / 2;
			outimg[(j*xFactor+3)+(width*xFactor*i*yFactor)+2] = (inimg[j+(i*width)+2] + inimg[(j+3)+(i*width) + 2]) / 2;

			outimg[(j*xFactor)+(width*xFactor*i*yFactor)+(width*xFactor*3)] = (inimg[j+(i*width)] + inimg[j+((i+3)*width)]) / 2; 
			outimg[(j*xFactor)+(width*xFactor*i*yFactor)+(width*xFactor*3)+1] = (inimg[j+(i*width)+1] + inimg[j+((i+3)*width)+1]) / 2; 
			outimg[(j*xFactor)+(width*xFactor*i*yFactor)+(width*xFactor*3)+2] = (inimg[j+(i*width)+2] + inimg[j+((i+3)*width)+2]) / 2; 

			outimg[(j*xFactor+3)+(width*xFactor*i*yFactor)+(width*xFactor*3)] = (inimg[j+(i*width)] + inimg[(j+3)+(i*width)] +inimg[j+((i+3)*width)] + inimg[(j+3) + ((i+3)*width)]) / 4;
			outimg[(j*xFactor+3)+(width*xFactor*i*yFactor)+(width*xFactor*3)+1] = (inimg[j+(i*width)+1] + inimg[(j+3)+(i*width)+1] +inimg[j+((i+3)*width)+1] + inimg[(j+3) + ((i+3)*width)+1]) / 4;
