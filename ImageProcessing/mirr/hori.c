...
void main(int argc, char** argv)
{
	...
	for(i = 0; i < height; i++) {
		for(j = 0; j < width; j++) {
			int idx = width-j;
			outimg[(idx*3-1)+(i*width*3)-2] = inimg[j*3+(i*width*3)+0];
			outimg[(idx*3-1)+(i*width*3)-1] = inimg[j*3+(i*width*3)+1];
			outimg[(idx*3-1)+(i*width*3)] = inimg[j*3+(i*width*3)+2];
		};
	};
