...
void hough_coord(Mat image, Mat& acc_mat, double rho, double theta, Point pt)
{
...
void acc_mask(Mat acc_mat, Mat& acc_dst, Size size, int thresh)
{
	...
	for (int r = h_m.y; r < acc_mat.rows - h_m.y; r++) {
		for (int t = h_m.x; t < acc_mt.cols - h_m.x; t++)
		{
...
void sort_lines(Mat lines, vector<Vec2f>& s_lines, double rho, double theta, int thresh)
{
